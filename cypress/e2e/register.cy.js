describe('template spec', () => {
  beforeEach(() => {
    ...
    it('user can access register page', () => {
        ...
        cy.get('.btn-google').contains('Already have an account? Login!')
        cy.get('.small').contains('Forgot Password?')
    ...
    it('user can not register using password without symbol', () => {
        cy.get('.user > :nth-child(2) > .form-control').type('Atmayanti')
        cy.get(':nth-child(3) > .form-control').type('Atmayanti@gmail.com')
        cy.get('.row > :nth-child(1) > .form-control').type('Aa1111')
        cy.get('.row > :nth-child(2) > .form-control').type('Aa1111')
        cy.get('.btn-primary').contains('Register Account').click()
        cy.get('.mb-3 > .invalid-feedback > strong').contains('The password must contain at least one symbol.')
    })
